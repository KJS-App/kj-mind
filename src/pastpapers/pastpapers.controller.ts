...
import { PaginatedResponse } from './types/pastpaper.types';
...
async getPapers(
  ...
  //get paper by id
  @Get("getPaperById")
  async getPaperById(@Query('paperId') paperId: string): Promise<IPastPaper | null> {
  ...
  @Post("updatePaper")
  async updatePaper(@Query('paperId') paperId: string, @Body() body: Partial<IPastPaper>): Promise<string> {
